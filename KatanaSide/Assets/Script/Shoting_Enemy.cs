...
{
    [Header("적 캐릭터 속성")]
    public float detctionRange = 10f;   //플레이어를 감지할 수 있는 최대 거리
    public float shootingInterval = 2f; //미사일 발사 사이의 대기 시간
    public GameObject missilePrefab;    //바사할 미사일 프리팹
    [Header("참조 컴포넌트")]
    public Transform firePoint; //미사일이 발사될 위치
    private Transform player;   //플레잉어의 위치 정보
    ...
    void Start()
    {
        ...
        shootTimer = shootingInterval;  //타이머 최기화
    }


    void Update()
    {
        if (player == null) return; //플레이어가 없으면 로직을 수행하지 않음
